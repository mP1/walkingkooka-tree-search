...
final class SearchQueryLeafAttributeValueDoesntContains extends SearchQueryLeafAttribute {
    ...
    public SearchQuery not() {
        return SearchQuery.attributeValueContains(this.value, this.attributeName, this.caseSensitivity);
    }

    @Override
    boolean test(final String text, final String attributeValue) {
        return false == this.caseSensitivity.contains(attributeValue, text);
    }

    @Override
    boolean canBeEqual(final Object other) {
        return other instanceof SearchQueryLeafAttributeValueDoesntContains;
    }
